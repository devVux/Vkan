...
GPU GPUSelector::select(int i) const {
	...
	return GPU(selected.device, gpu, indices);
}

const GPUSelector& GPUSelector::tellFor(const Window& window) {

	auto candidateDevices = ResourceFactory::tellGPUs(mContext)
